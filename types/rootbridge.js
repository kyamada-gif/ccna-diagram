...
(function (global) {
  ...
  var SPOTS = [
    { key: "pri", name: "ブリッジ優先度",
      ...
      use: "4台をくらべて、最も小さい数を探す。それが1台だけなら、そのスイッチが答え。同じ数が2台以上あるときは、次に MACアドレス を見る" },
    { key: "mac", name: "MACアドレス",
      mean: "機器につけられた番号。16進数（0〜9 と a〜f）で書く",
      use: "優先度が最も小さいスイッチが2台以上あるとき、その中で MACアドレス が小さい方が答え。左から1文字ずつくらべる。数字は文字より小さい（0 < 9 < a < f）" }
  ];
  ...
  var RULES = [
    { key: "pri", cond: "最も小さい優先度のスイッチが、1台だけ",
      verdict: "ブリッジ優先度で決まる",
      why: "優先度が最も小さいスイッチが1台しかないので、そこで決まる",
      look: ["ブリッジ優先度"],
      steps: function (v) {
    ...
    { key: "mac", cond: "最も小さい優先度のスイッチが、2台以上",
      verdict: "MACアドレスで決まる",
  ...
  var GLOSS = {
    "ブリッジ優先度で決まる": "優先度の数が最も小さいスイッチが、代表になる",
