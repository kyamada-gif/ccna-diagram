...
(function (global) {
  ...
  var SPOTS = [
    { key: "pri", name: "ブリッジ優先度",
      ...
      use: "各スイッチの優先度を比べて、値が最も小さいスイッチを探す。それが1台だけなら、そのスイッチがルートブリッジになる。同じ値のスイッチが2台以上あるときは、次に MAC アドレスを比べる" },
    { key: "mac", name: "MACアドレス",
      mean: "機器ごとに割り当てられた固有の番号。16進数（0〜9 と a〜f）で書く",
      use: "優先度が最も小さいスイッチが2台以上あるときに見る。その中で MAC アドレスが小さいほうがルートブリッジになる。比べ方は左の文字から順で、差がついたところで決まる。数字は英字より小さい（0 < 9 < a < f）" }
  ];
  ...
  var RULES = [
    { key: "pri", cond: "優先度が最も小さいスイッチが1台だけ",
      verdict: "ブリッジ優先度で決まる",
      why: "優先度が最も小さいスイッチが1台しかないので、MAC アドレスを見るまでもなくルートブリッジが決まる",
      look: ["ブリッジ優先度"],
      steps: function (v) {
    ...
    { key: "mac", cond: "優先度が最も小さいスイッチが2台以上",
      verdict: "MACアドレスで決まる",
      why: "優先度では差がつかないので、その中で MAC アドレスが小さいほうがルートブリッジになる",
      look: ["MACアドレス"],
      steps: function (v) {
        return [["最も小さい優先度", v.lowest], ["その優先度のスイッチ", v.tied.length + " 台"],
  ...
  var GLOSS = {
    ...
    "MACアドレスで決まる": "優先度が並んだときの決め方。MAC アドレスが小さいほうがルートブリッジになる"
  };
   ...
   *   ブリッジ優先度 を確認します。ここでルートブリッジは決まりますか。
   *     ・ブリッジ優先度 で SW4 に決まる
