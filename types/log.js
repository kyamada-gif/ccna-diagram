...
(function (global) {
  ...
  var SPOTS = [
    { key: "kind", name: "ログの種類",
      re: /-\d-([A-Z]+)/,
      mean: "ログの見出しの、最後の英字。何のログかを表す",
      use: "COLL なら衝突（ぶつかり）のログ。ここでログの中身が決まるので、まずここを見る" },
    { key: "word", name: "本文の言葉",
      re: /(excessive collisions|Excessive collisions)/i,
      mean: "ログの本文に出ている言葉。何が多すぎるのかが書いてある",
      use: "excessive collisions なら「衝突が多すぎる」。送り直しを16回しくじると、そのフレームは捨てられる" }
  ];
  ...
  /* ── 判定ルール ─────────────────────────── */
  var RULES = [
    { key: "coll", cond: "ログに excessive collisions と出ている",
      verdict: "送信を16回しくじると、そのフレームは捨てられる",
      why: "衝突が多すぎるログ。送り直しを16回続けてしくじると、そのフレームはあきらめて捨てられる",
      look: ["ログの種類", "本文の言葉"],
      steps: function (v) {
  ...
  var GLOSS = {
    "送信を16回しくじると、そのフレームは捨てられる":
      "ぶつかるたびに送り直すが、16回続けてだめなら、あきらめる決まりになっている"
  };
