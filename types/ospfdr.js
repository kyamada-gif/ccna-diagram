...
(function (global) {
  ...
  var SPOTS = [
    { key: "pri", name: "OSPF の優先度",
      ...
      use: "4台をくらべて、最も大きい数を探す。それが1台だけなら、そのルータが答え。同じ数が2台以上あるときは、次に ルータ ID を見る。スイッチの代表は数が小さい方が勝つが、ルータの代表は大きい方が勝つ" },

    { key: "rid", name: "ルータ ID",
      mean: "ルータに割り当てられた識別番号。ピリオドで区切った4つの数で書く",
      use: "優先度が最も大きいルータが2台以上あるとき、その中で ルータ ID が大きい方が答え。左の数から順にくらべる。図に ルータ ID が書いていないときは Loopback の番号、それも無ければ IP アドレスが ルータ ID になる" }
  ];
  ...
  var RULES = [
    { key: "pri", cond: "最も大きい優先度のルータが、1台だけ",
      verdict: "OSPF の優先度で決まる",
      why: "優先度が最も大きいルータが1台しかないので、そこで決まる",
      look: ["OSPF の優先度"],
      steps: function (v) {
      ...
      no: function (v) {
        return "最も大きい優先度 " + v.top + " のルータが " + v.tied.length +
               " 台あるので、これだけでは決まらない";
      },
      test: function (v) { return v.tied.length === 1; } },

    { key: "rid", cond: "最も大きい優先度のルータが、2台以上",
      verdict: "ルータ ID で決まる",
  ...
  var GLOSS = {
    "OSPF の優先度で決まる": "優先度の数が最も大きいルータが、代表になる",
