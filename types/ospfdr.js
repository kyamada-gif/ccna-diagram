...
(function (global) {
  ...
  var SPOTS = [
    { key: "pri", name: "OSPF の優先度",
      mean: "そのルータが代表になりたい度合い。数が大きいほど強い",
      use: "4台をくらべて、いちばん大きい数を探す。それが1台だけなら、そのルータが答え。同じ数が2台以上あるときは、次に ルータ ID を見る。スイッチの代表は数が小さい方が勝つが、ルータの代表は大きい方が勝つ" },

    { key: "rid", name: "ルータ ID",
      mean: "ルータに付いている名札の番号。ピリオドで区切った4つの数で書く",
      use: "優先度がいちばん大きいルータが2台以上あるとき、その中で ルータ ID が大きい方が答え。左の数から順にくらべる。図に ルータ ID が書いていないときは Loopback の番号、それも無ければ IP アドレスが ルータ ID になる" }
  ];
  ...
  var RULES = [
    { key: "pri", cond: "いちばん大きい優先度のルータが、1台だけ",
      verdict: "OSPF の優先度で決まる",
      why: "優先度がいちばん大きいルータが1台しかないので、そこで決まる",
      look: ["OSPF の優先度"],
      steps: function (v) {
        return [["いちばん大きい優先度", v.top],
                ["その優先度のルータ", v.tied.length + " 台"],
                ["4台の優先度", listPri(v)]];
      },
      no: function (v) {
        return "いちばん大きい優先度 " + v.top + " のルータが " + v.tied.length +
               " 台あるので、これだけでは決まらない";
      },
      test: function (v) { return v.tied.length === 1; } },

    { key: "rid", cond: "いちばん大きい優先度のルータが、2台以上",
      verdict: "ルータ ID で決まる",
      why: "優先度が同じなので、その中で ルータ ID が大きい方になる",
      look: ["ルータ ID"],
      steps: function (v) {
        return [["いちばん大きい優先度", v.top],
                ["その優先度のルータ", v.tied.length + " 台"],
                ["くらべる ルータ ID", listRid(v)]];
      },
      no: function (v) {
        return "いちばん大きい優先度 " + v.top +
  ...
  var GLOSS = {
    "OSPF の優先度で決まる": "優先度の数がいちばん大きいルータが、代表になる",
    "ルータ ID で決まる": "優先度が同じときの決着のつけ方。番号が大きい方が勝つ"
  };
  ...
  function walkQ(st, v, shuffle) {
    ...
    return { ask: look + " を見ます。ここで DR は決まりますか。",
  ...
  var spec = {
    ...
    card: "read",
    name: "DR の決まり方",
    note: "同じ線につながったルータの中から、代表を1台決める",
    obj: "3.4",
